...
const CheckOutForm = () => {
  ...
  useEffect(() => {
    ...
    return;
  }, [deliveryCost, selectedCountry, totalWeight]);
      ...
      <Form method="post">
        ...
        {paymentMethod === "cash" && (
          <button
            ...
            className={`px-4 py-1 rounded-lg mx-auto w-full inline-block mt-4`}
          >
            Submit Your Order
          </button>
        )}
...
export const checkOutFormAction = async ({ request }) => {
  ...
  console.log(JSON.parse(data.get("cart")));
  ...
  //
  console.log(order);
  const response = await fetch(`${baseURL}/orders/${userIdentifier}.json`, {
    method: "post",
    headers: {
